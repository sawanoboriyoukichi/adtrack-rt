...
// ローカル日付をYYYY-MM-DD文字列に変換（toISOString()はUTC変換されるため使わない）
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
        ...
        {PRESETS.map(preset => (
          <button key={preset.label} onClick={() => handlePreset(preset)} disabled={loading}
            style={{
              ...
              border: (!isCustom && active?.label === preset.label) ? '1px solid #1a8fc1' : '1px solid #ccc',
              backgroundColor: (!isCustom && active?.label === preset.label) ? '#1a8fc1' : 'white',
              color: (!isCustom && active?.label === preset.label) ? 'white' : '#555',
              cursor: loading ? 'default' : 'pointer',
              fontWeight: (!isCustom && active?.label === preset.label) ? 'bold' : 'normal',
        ...
        {onRefresh && (
          <button onClick={onRefresh} disabled={loading}
            style={{
              marginLeft: '4px', padding: '5px 14px', fontSize: '12px', borderRadius: '4px',
              border: '1px solid #1a8fc1',
              backgroundColor: loading ? '#ccc' : '#1a8fc1',
              color: 'white',
              cursor: loading ? 'default' : 'pointer',
              opacity: loading ? 0.6 : 1,
            }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
        ...
        <input type="date" value={customStart} max={customEnd || today} onChange={e => setCustomStart(e.target.value)} disabled={loading}
          style={{ padding: '4px 8px', fontSize: '12px', border: `1px solid ${isCustom ? '#1a8fc1' : '#ccc'}`, borderRadius: '4px', color: '#333', cursor: loading ? 'default' : 'text', outline: 'none', backgroundColor: isCustom ? '#e8f4fb' : 'white' }} />
        ...
        <button onClick={applyCustom} disabled={loading || !customStart || !customEnd || customStart > customEnd}
          style={{
            padding: '5px 14px', fontSize: '12px', borderRadius: '4px', border: '1px solid #e67e22',
            backgroundColor: (loading || !customStart || !customEnd || customStart > customEnd) ? '#eee' : '#e67e22',
            color: (loading || !customStart || !customEnd || customStart > customEnd) ? '#aaa' : 'white',
            cursor: (loading || !customStart || !customEnd || customStart > customEnd) ? 'default' : 'pointer',
        ...
        {isCustom && (
          <span style={{ fontSize: '11px', color: '#e67e22', fontWeight: 'bold', whiteSpace: 'nowrap' }}>
            📅 {startDate} 〜 {endDate}
          </span>
        )}
      </div>
