...
export default function MicroConversionSelector({ events, selected, onChange }) {
  ...
  const visibleEvents = events.filter(e =>
    ...
    (search === '' || e.name.toLowerCase().includes(search.toLowerCase()) || getEventLabel(e.name).toLowerCase().includes(search.toLowerCase()))
  );
  ...
  const moveUp = (idx) => { if (idx === 0) return; const arr = [...selected]; [arr[idx-1], arr[idx]] = [arr[idx], arr[idx-1]]; onChange(arr); };
  const moveDown = (idx) => { if (idx === selected.length - 1) return; const arr = [...selected]; [arr[idx], arr[idx+1]] = [arr[idx+1], arr[idx]]; onChange(arr); };
  const clearAll = () => { onChange([]); setActivePresetId(null); };

  const applyPreset = (preset) => { onChange([...preset.events]); setActivePresetId(preset.id); setIsOpen(false); };
  ...
  const saveCurrentPreset = () => {
    ...
    setSavedPresets(updated); persistPresets(updated);
    setActivePresetId(newPreset.id); setSaveName(''); setSavingMode(false);
  };
  ...
  const overwritePreset = (preset) => {
    ...
    setSavedPresets(updated); persistPresets(updated); setActivePresetId(preset.id);
  };
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', position: 'relative' }} ref={dropdownRef}>
      ...
      <button onClick={() => setIsOpen(!isOpen)}
        ...
        <span>📊 マイクロCV選択</span>
        {selected.length > 0 && (
          <span style={{ backgroundColor: isOpen ? 'white' : '#1a8fc1', color: isOpen ? '#1a8fc1' : 'white', borderRadius: '10px', padding: '1px 7px', fontSize: '11px', fontWeight: 'bold' }}>{selected.length}</span>
        )}
        ...
        <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '4px', backgroundColor: 'white', border: '1px solid #ccc', borderRadius: '6px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)', zIndex: 2000, width: '340px', maxHeight: '580px', display: 'flex', flexDirection: 'column' }}>
              ...
              {savedPresets.map(preset => (
                <div key={preset.id} style={{ padding: '6px 14px', display: 'flex', alignItems: 'center', gap: '6px', borderBottom: '1px solid #edf2f7', backgroundColor: activePresetId === preset.id ? '#e8f4fb' : 'transparent' }}>
                  <button onClick={() => applyPreset(preset)} style={{ flex: 1, textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                    <div style={{ fontSize: '12px', fontWeight: activePresetId === preset.id ? 'bold' : 'normal', color: activePresetId === preset.id ? '#1a8fc1' : '#333' }}>{activePresetId === preset.id && '▶ '}{preset.name}</div>
                    <div style={{ fontSize: '10px', color: '#aaa', marginTop: '1px' }}>{preset.events.slice(0,3).map(getEventLabel).join(' / ')}{preset.events.length > 3 ? ` 他${preset.events.length-3}件` : ''}</div>
                  </button>
                  {activePresetId === preset.id && <button onClick={() => overwritePreset(preset)} style={{ ...btnBase, backgroundColor: '#e8f4fb', color: '#1a8fc1', fontSize: '10px' }}>更新</button>}
                  <button onClick={() => deletePreset(preset.id)} style={{ ...btnBase, backgroundColor: '#fff0f0', color: '#e53935' }}>✕</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {visibleEvents.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center', color: '#888', fontSize: '13px' }}>{events.length === 0 ? 'データ取得中...' : '見つかりません'}</div>
              ...
              return (
                <div key={ev.name} onClick={() => toggleEvent(ev.name)}
                  ...
                  <span style={{ fontSize: '11px', color: '#aaa', flexShrink: 0 }}>{ev.count.toLocaleString()}</span>
                </div>
              );
            })}
          </div>
            ...
            {savingMode ? (
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <input type="text" value={saveName} onChange={e => setSaveName(e.target.value)} placeholder="例: LINE登録キャンペーン" autoFocus onKeyDown={e => e.key === 'Enter' && saveCurrentPreset()}
                    style={{ flex: 1, padding: '5px 10px', fontSize: '12px', border: '1px solid #1a8fc1', borderRadius: '4px' }} />
                  <button onClick={saveCurrentPreset} disabled={!saveName.trim()} style={{ padding: '5px 12px', fontSize: '12px', backgroundColor: saveName.trim() ? '#1a8fc1' : '#ccc', color: 'white', border: 'none', borderRadius: '4px', cursor: saveName.trim() ? 'pointer' : 'default' }}>保存</button>
                  <button onClick={() => { setSavingMode(false); setSaveName(''); }} style={{ padding: '5px 10px', fontSize: '12px', backgroundColor: '#eee', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>×</button>
                </div>
              </div>
            ) : (
